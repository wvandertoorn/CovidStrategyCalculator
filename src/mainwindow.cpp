...
QWidget *MainWindow::initialize_tab_strategy()
{
    ...
    this->mode_ComboBox->addItems(QStringList{"contact management", "isolation", "incoming travelers"});
    ...
    this->pre_risk = create_parameter_DoubleSpinBox(this, 0, 100, 3, 100);
    ...
    gridLayout->addWidget(new QLabel(tr("Initial risk [%]: ")), 1, 0);
...
{
    // variables
    QLabel *label_tau_inc = new QLabel(tr("Mean duration of incubation period [day]:"));
    QLabel *label_percentage_predetect = new QLabel(tr("Percentage thereof pre-detectable [%]:"));
    QLabel *label_tau_symp = new QLabel(tr("Mean duration of symptomatic period [day]:"));
    QLabel *label_asymp = new QLabel(tr("Percentage of asymptomatic infections [%]:"));
    QLabel *label_tau_post = new QLabel(tr("Mean duration of post-symptomatic, detectable period [day]:"));
    ...
    for (int i = 0; i < 3; ++i)
    {
        connect(v[i], &QCheckBox::stateChanged, [=](int){
                                                            if (v[i]->isChecked()){
                                                                ...
                                                                this->pre_risk->setValue(pre_procedure_risk* 100.);
